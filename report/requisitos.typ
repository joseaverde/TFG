...
#let config = srs.make-config(
  ...
  classes: (
    ...
    srs.make-class(
      ...
      "Test",
      namer: srs.defaults.incremental-namer-maker(
        prefix: (tag, root-class-name, class-name, separator) => { tag.join("") },
        start: 1,
        width: 2,
      ),
      identifier: srs.defaults.identifier-maker(prefix: none),
...
#let reqs = srs.create(
  ...
  make-item(
    ...
    (
      Descripción: [
        El sensor debe leer muestras a un ritmo constante, es decir, el
        intervalo entre lecturas debe ser el mismo.],
      Necesidad: "h",
      Prioridad: "h",
      Estabilidad: "i",
      Verificabilidad: "m",
