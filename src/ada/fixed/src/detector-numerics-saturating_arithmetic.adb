...
package body Detector.Numerics.Saturating_Arithmetic with SPARK_Mode is
   ...
   type Fixed_Long_Long_Integer is
      delta 1.0
      range -2.0 ** (Bits * 4 - 1) .. 2.0 ** (Bits * 4 - 1) - 1.0 with
   Size => Bits * 4;
   ...
   function Generic_Saturated_Multiplication (
      ...
      Result_First : constant Fixed_Long_Long_Integer := (
         Result_Fixed_Type'First
         / Result_Fixed_Type'(Result_Fixed_Type'Delta));
      Result_Last : constant Fixed_Long_Long_Integer := (
         Result_Fixed_Type'Last / Result_Fixed_Type'(Result_Fixed_Type'Delta));
      Result_Real_Den : constant Fixed_Long_Long_Integer := (
         1.0 / Result_Fixed_Type'(Result_Fixed_Type'Delta));

      Result     : Result_Fixed_Type'Base;
      Result_Num : Fixed_Long_Long_Integer;
      Result_Den : Fixed_Long_Long_Integer;
      Factor     : Fixed_Long_Long_Integer;
   begin
      pragma Assert (Left_Den > 0.0);
      pragma Assert (Right_Den > 0.0);
      pragma Assert (Result_Real_Den > 0.0);
      ...
      Result_Den := Left_Den * Right_Den;
      pragma Assert (Result_Den > 0.0);
      ...
      if Result_Num < Result_First then
         ...
         Result := Result_Num / Result_Real_Den;
